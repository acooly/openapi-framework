				...
				<div class="container-main">
					...
					<div class="doc-content spec">
                        ...
                        <ol>
                            ...
                            <li><strong>4. 同步返回结果</strong>：服务平台同步响应商户系统服务同步处理结果，一般情况返回的结果如果是成功则表示请求已接收或完成部分处理，需要商户系统等待异步通知业务最终处理结果；如果是失败则表示业务处理已终止，无后续异步通知。</li>
                            ...
                            <li><strong>8. 异步通知数据处理</strong>：商户系统在成功收到异步通知后，需要在响应体(http响应的body中)中打印输出”success” （不包含引号）表示商户系统已成功收到异步通知。如果商户反馈给服务平台的字符不是success这7个字符，服务平台服务器会不断重发通知，直到超过24小时。一般情况下，24小时以内完成8次通知（通知的间隔频率一般是：2m,10m,10m,1h,2h,6h,15h）</li>
                        </ol>
                        ...
                        <p>同步转异步的主体交互模式完全同异步通讯。不同的是异步通知是可选的，只有在同步响应返回resultCode=EXECUTE_PROCESSING的时候才会有异步通知。具体根据<q>异步通讯</q>第5步的时候商户系统需要对返回的resultCode进行如下判断：</p>

                        <ul>
                            <li>如果resultCode=EXECUTE_PROCESSING,那么表示该业务返回时没有完成最终处理，当前正在处理中，需要等待后续异步通知最终结果；</li>
                            <li>如果resultCode=EXECUTE_SUCCESS表示该业务已经完成最终处理，后续无异步通知。</li>
...

{
    ...
    &quot;partnerId&quot;:&quot;20141229020000062199&quot;
                            ...
                            <li><p><strong>通知报文</strong>：通知报文是网关完成处理后，主动请求商户端通知处理的结果。其中包括同步通知和异步通知，同步通知的地址为请求报文中传入的returnUrl, 异步通知的地址为请求报文中商户端传入的notifyUrl。同步通知采用http/redirect方式跳转returnUrl通知，采用http-get方式，异步通知采用http-post方式后台同步请求通知商户端。</p>

                                <p>同步通知报文实例：</p>

                                <pre><code>http://www.merchant.com/returnUrl.html?key1=value1&amp;key2=value2&amp;key3=%E6%9C%8D%E5%8A%A1%E6%A1%86%E6%9E%B6</code></pre>

                                <blockquote>
                                    <p>其中：<a href="http://www.merchant.com/returnUrl.html">http://www.merchant.com/returnUrl.html</a> 是商户端请求时传入的returnUrl.</p>
                                </blockquote>

                                <p>异步通知报文实例：</p>

                                <pre class="line-numbers"><code class="language-http">POST /openapi/notify.html HTTP/1.1
Content-Length: 529
Content-Type: application/x-www-form-urlencoded; charset=UTF-8
Host: www.merchant.com
Connection: Keep-Alive

amount=1000.00&amp;...signType=MD5&amp;notifyUrl=http%3A%2F%2Fwww.merchant.com%3A8090%2Fopenapi%2Fnotify&amp;notifyTime=2016-06-02%2012:12:12&amp;version=1.0</code></pre></li>
                        </ul>
                        ...
                        <ul>
                            ...
                            <li>“C”：为有条件选择型(Conditional),表示在符合条件的情况下,必须填写此数据项,不符
                                合条件的情况下,可以不填写此数据项，相关条件一般会在具体报文说明中明确说明。</li>
                        </ul>
                        ...
                        <blockquote>
                            <p>幂等性：orderNo是交易级（订单支付交易等）幂等要素，表示服务会根据orderNo参数维持服务的一致性（注意不是相同的结果，是行为一致）。当用户多次传入相同的orderNo请求交易时，服务会根据orderNo对应的实际交易的状态按一致的处理行为进行处理，比如：该订单对应的交易已处理完成，后续所有相同订单的请求对直接返回成功并处理完成，如果该订单对应的交易正在处理中或挂起中，则用户再次请求会触发业务继续进行~~，并返回当前的状态。</p>
                        </blockquote>
                        ...
                        <blockquote>
                            <p>结果判断：<br>
                                * success返回的标志只是标志该服务是否执行成功，并不一定确定该服务对应的业务处理成功，请根据具体接口进行判断。比如：异步提现接口，请求后的同步响应success=true,resultCode=EXECUTE_PROCESSING 则表示：异步提现提交成功，正常处理中，提现并没有成功完成。<br>
                                * resultCode=EXECUTE_SUCCESS和resultCode=EXECUTE_PROCESSING表示处理成功，其他表示失败。</p>
                        </blockquote>

                        <h4 id="toc_17">公共通知报文</h4>

                        <p>网关在设计处理通知报文时，为提高商户端的使用简便和体验，服务的同步通知（跳转通知）和异步通知采用完全相同的结构，只是在同步通知和异步通知时部分数据项填充不一定一致，请跟进具体api服务及业务进行处理。所以我们同步和异步通知的公共报文部分是完全相同的。</p>
                        ...
                        <p>所有API服务都是通过错误码标识处理结果，包括同步响应，跳转同步通知和异步通知我们采用这一统一的原则。错误码的表示方式采用三元错误消息方式，由resultCode,resultMessage和resultDetai三个数据项表示唯一的错误消息，其中resultDetail为可选。</p>
                        ...
                        <ul>
                            <li><p><strong>摘要安全模式</strong>：通讯报文的签名，验签使用摘要算法（MD5，Sha1Hex和Sha256Hex等）实现和数据加解密采用ASE对称加密算法，商户收到的秘钥为32字节的字符串秘钥，我们命名为secrtKey，用于报文的摘要签名，验签和对称加解密的秘钥。 例如：c9cef22553afujh64b04a012f9cb8ea9</p></li>
                            <li><p><strong>证书安全模式</strong>：如果商户采用的是证书安全模式，通讯报文的签名，验签和加解密都使用RSA算法。商户会收到一对秘钥，包含商户证书（${partnerId}.pfx）和网关证书(server.cer)的两个文件。商户端的秘钥对(文件)为：</p>

                                <ul>
                                    <li><p>商户证书: 商户的keystore证书（PFX格式），包含用户的私钥和公钥证书，主要用于请求报文签名和收到报文的数据解密，例如：20160010101010101.pfx, 如果是联调试环境，一般命名为: 20160010101010101.snet.pfx，同时会邮件发送给商户商户证书文件的保护密码(keystorePassword)。</p></li>
                        ...
                        <blockquote>
                            <p>注意：报文的签名数据默认为全签，以java为例子，建议直接通过request.getParameters()获取全部的参数，我们API服务的接口通信报文中不存在同名参数的多个值，所有该方法返回的所有参数值都可以直接按没有数组的情况处理。</p>
                        </blockquote>
    ...
    &quot;tradeAmount=100&quot;
                        ...
                        <blockquote>
                            <p>注意：<br>
                                ...
                                3. 待签字符串中的特殊字符无需签名，根据HTTP协议，传递参数的值中如果存在特殊字符（如：&amp;、@等）和中文字符，那么该值需要做UTF-8编码的URL-Encoding，这样请求接收方才能接收到正确的参数值。这种情况下，待签名数据应该是原生值而不是encoding之后的值。例如：调用某接口需要对请求参数email进行数字签名，那么待签名数据应该是email=<a href="mailto:test@msn.com">test@msn.com</a>，而不是email=test%40msn.com。</p>
                        </blockquote>

                        <p><strong>复杂报文签名</strong></p>

                        <p>所有的请求报文和通知报文（异步通知，同步跳转通知）采用FORM方式的key/value方式组报的主要是为便于form表单提交，如果API服务某个数据项是复杂对象(Object)类型或数组类型(Array)，则需要把该项的值转换为标准JSON字符串后作为签名的参数值。</p>

                        <ol>
                            <li><p>请求报文的签名</p>

                                <p>我们使用一个示例来进行说明，在标准支付接口中创建订单时，会要求传入的商品列表字段：goodlist，需要请求商户传递本次支付的所有商品信息（多个），每个商品信息为一个对象结构，包括：name，price，quantity等数据项，这个数据线的类型为复杂类型Array，那么:</p>
                                ...
                                <pre><code>goodsInfos =%5B%7B%22goodType%22%3A%22actual%22%2C%22name%22%3A%22%E5%A4%A9%E5%AD%90%E7%B2%BE   %E5%93%811%22%2C%22price%22%3A400.00%2C%22quantity%22%3A1%2C%22referUrl%22%3A%22http%3A%2F  %2Facooly.cn%2Ftianzi%22%7D+%2C%7B%22goodType%22%3A%22actual%22%2C%22name%22%3A%22%E5%A4%A9%E5%AD   %90%E7%B2%BE%E5%93%812%22%2C%22price%22%3A400.00%2C%22quantity%22%3A1%2C%22referUrl%22%3A%22http%3A %2F%2Facooly.cn%2Ftianzi%22%7D%5D</code></pre></li>
                            <li><p>通知报文的验签<br>
                                复杂结构时生成待签字符串的方式与（1. 请求报文的签名）相同。</p></li>
                            <li><p>响应报文的验签<br>
                                响应报文采用在http协议Body体写入JSON结构体方式响应数据。请求方收到的同步响应为标准JSON结构体。如果是单级结构（类似   key/value）时，则采用通用验证签方式组织待签字符串。如果是多级结构，则需要把二级以下的Json结构体以字符串方式作为一级属性  的数据项参与验证签名。</p>
    ...
    //……
                                ...
                                <pre class="line-numbers"><code class="language-http">goodsInfos=[{&quot;goodType&quot;:&quot;actual&quot;,&quot;name&quot;:&quot;天子精品1&quot;,&quot;price&quot;:400.00,&quot;quantity&quot;:1,&quot;referUrl&quot;:&quot;http://acooly.cn/tianzi&quot;} ,{&quot;goodType&quot;:&quot;actual&quot;,&quot;name&quot;:&quot;天子精品2&quot;,&quot;price&quot;:400.00,&quot;quantity&quot;:1,&quot;referUrl&quot;:&quot;http://acooly.cn/tianzi&quot;}]</code></pre></li>
                        </ol>
                        ...
                        <ol>
                            ...
                            <li><p><strong>验证签名</strong>：验证签名是在商户端收到网站的同步响应报文或通知报文后，对这些报文进行签名验证。原则为，使用“2. <strong>签名计算</strong>”的方法对报文计算签名值为signClient,然后与报文中传入的sign参数值（服务器端计算的签名）进行字符串比较，如果相同则表示验证签名通过，报文在传输过程中未被篡改。</p></li>
                        </ol>

                        <h4 id="toc_26">证书签名和验签</h4>

                        <p>证书签名和验签基于商户证书，使用非对称加密算法RSA实现。如果我们选择使用证书签名方式，则要求商户在开通服务时选择证书加密选项，商户完成开户时，会收到包含商户证书（pfx）和网关证书(cer)的两个文件，这两个文件是完成我们签名和后续数据加密的秘钥对文件。同时另外一种获取秘钥文件的方式是登录开放平台，自助下载证书文件。</p>
                        ...
                        <ul>
                            <li>商户证书格式为pfx格式,keystore类型（keystoreType）为PKCS12，keystore保护密码(keystorePassword)联调环境为6个1（111111），计算RSA签名时，需要从pfx文件中获取私钥数据，你可以使用各语言的通用API加载并获取（推荐方案），也可以通过openssl命令导出私钥文件。</li>
                        ...
                        <ol>
                            ...
                            <li><strong>签名计算</strong>: 签名的计算使用标准RSA签名算法，请根据商户端语言环境选择具体的实现，使用商户证书的<strong>商户私钥</strong>对请求报文进行签名计算，计算出的结果为256字节（byte数组）的签名数据,请对签名数据进行标准base64编码，最后得到344字节的字符串则为报文的签名。</li>
...
&lt;/dependency&gt;</code></pre>
    ...
    public static boolean verifyMD5(Map&lt;String, String&gt; data, String key, String verifySign) {
        ...
        return verifySign.equals(signature);
                        ...
                        <blockquote>
                            <p>GET方式请求不需要进行URLEncoding，如果是POST方式提交不用进行编码处理，POST-FORM模式需要指定Header:  Content-Type: application/x-www-form-urlencoded; charset=UTF-8</p>
                        </blockquote>
