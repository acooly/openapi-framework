...
import com.yiji.framework.openapi.service.persistent.dao.NotifyMessageDao;
...
public class NotifyMessageDaoImpl extends AbstractJdbcTemplateDao implements NotifyMessageDao {
    ...
    private String getSelectSql() {
        return "select id,gid,partner_id as partnerId,message_type as messageType,service,version,url,content,send_count as sendCount,"
                ...
                + "resp_info as respInfo,request_no as requestNo, merch_order_no as merchOrderNo from " + TABLE_NAME;
    }

    @Override
    public PageInfo<NotifyMessage> query(PageInfo<NotifyMessage> pageInfo, Map<String, Object> map,
    ...
    public void insert(final NotifyMessage notifyMessage) {
        final String sql = "insert into " + TABLE_NAME
                + "(" + (getDbType() == DbType.oracle ? "id," : "") + "gid,partner_id,message_type,service,version,url,content,send_count,next_send_time,status,execute_status,create_time,update_time,resp_info,request_no,merch_order_no) "
                + " values(" + (getDbType() == DbType.oracle ? "seq_api_notify_message.nextval," : "") + "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public void updateStatus(NotifyMessage notifyMessage) {
        String sql = "update " + TABLE_NAME
                + " set update_time = " + getSqlDate() + " ,status = ?,execute_status = ?,send_count = ?,next_send_time=?, resp_info=?"
                + " where id = ?";
        jdbcTemplate.update(sql, notifyMessage.getStatus().code(), notifyMessage.getExecuteStatus().code(),
                notifyMessage.getSendCount(), notifyMessage.getNextSendTime(), notifyMessage.getRespInfo(), notifyMessage.getId());
    }
    ...
    public void updateForManage(NotifyMessage notifyMessage) {
        ...
        jdbcTemplate.update(sb.toString(), notifyMessage.getUrl(), notifyMessage.getStatus().code(),
                notifyMessage.getNextSendTime(), notifyMessage.getId());
    }
    ...
    public int updateProccessingStatus(NotifyMessage notifyMessage) {
        String sql = "update " + TABLE_NAME + " set update_time = " + (getDbType() == DbType.mysql ? "now()" : "sysdate") + " ,execute_status = '"
                + TaskExecuteStatus.Processing.code() + "' where id = ? and status = '" + TaskStatus.Waitting.code() + "' and execute_status = '" + TaskExecuteStatus.Unprocessed.code() + "'";
    ...
    public void updateProccessingStatus(List<NotifyMessage> notifyMessages) {
        ...
        String sql = "update " + TABLE_NAME + " set update_time = " + (getDbType() == DbType.mysql ? "now()" : "sysdate") + " ,execute_status = '"
                + TaskExecuteStatus.Processing.code() + "' where id in " + idsStr;
    ...
    private RowMapper<NotifyMessage> rowMapper = new RowMapper<NotifyMessage>() {
        @Override
        public NotifyMessage mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            notifyMessage.setParameters(JSON.parseObject(notifyMessage.getContent(), mapTypeReference));
