...
public class WithdrawRequest extends ApiAsyncRequest {
    ...
    @OpenApiField(desc = "提现金额", constraint = "提现金额，格式为保留两位小数的元，如: 2.00,200.05", demo = "200.15", ordinal = 3)
    private Money amount;

    @Length(max = 15, message = "银行编码不能为空，最大长度为15字符")
    @OpenApiField(desc = "银行编码", constraint = "请参考附录的银行编码列表", demo = "ABC", ordinal = 4)
    private String bankCode;

    @Length(max = 30, min = 15)
    @OpenApiField(desc = "银行卡号", demo = "6226998032873746", ordinal = 5)
    private String bankCardNo;


    @OpenApiField(desc = "到账方式", constraint = "可选值：<li>0: T+0</li><li>1: T+1</li><li>2: T+2</li>", demo = "1", ordinal = 6)
    @Length(max = 1, min = 1, message = "长度为1字符")
    private String delay;

    @OpenApiField(desc = "业务类型", demo = "BUSI2", ordinal = 7)
