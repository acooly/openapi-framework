...
import com.yiji.framework.openapi.common.message.ApiReturn;
...
import com.yiji.framework.openapi.core.marshall.ApiMarshall;
 ...
 * API返回报文抽象实现
 * <p/>
 * Created by zhangpu on 2015/11/19.
 */
public abstract class AbstractHttpFormJsonResponseMarshall<T, S extends ApiResponse> implements ApiMarshall<T, S> {
    ...
    protected void doSign(ApiResponse apiResponse, Map<String, String> signData) {
        ...
        // 服务认证失败
        if (resultCode != null && resultCode.equals(ApiServiceResultCode.UN_AUTHENTICATED_ERROR.getCode())) {
            return;
        }
        // 授权失败
        if (resultCode != null && resultCode.equals(ApiServiceResultCode.UN_AUTHORIZED_ERROR.getCode())) {
            return;
        }
