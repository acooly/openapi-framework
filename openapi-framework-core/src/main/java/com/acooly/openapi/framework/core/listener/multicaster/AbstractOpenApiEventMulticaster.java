 ...
 * @author qiubo@qq.com
 */
public class AbstractOpenApiEventMulticaster<E extends OpenApiEvent>
    implements OpenApiEventMulticaster<E> {
  private static final Logger logger =
      LoggerFactory.getLogger(AbstractOpenApiEventMulticaster.class);

  /** 设置监听器过多阀值警告,请确认是否在服务执行方法中设置监听器后正确删除监听器.请尽量在服务初始化时设置监听器 */
  private static final int INTERCEPTORS_THRESHOLD = 10;
  private static TaskExecutor taskExecutor =
      OpenApiThreadPoolTaskExecutorFactory.getThreadPoolTaskExecutor();
  ...
  public void addListener(ApiListener<E> listener) {
    ...
    if (listeners.contains(listener)) {
      logger.warn("服务:{} 重复设置拦截器:{}", this.getClass().getName(), listener.getClass().getName());
      return;
    }
    ...
    if (accept(event)) {
      for (final ApiListener listener : listeners) {
        if (listener instanceof SmartListenter) {
          boolean support = ((SmartListenter) listener).supportsEventType(event.getClass());
          if (!support) {
            continue;
          }
          OpenApiListener openApiListener =
              listener.getClass().getAnnotation(OpenApiListener.class);
