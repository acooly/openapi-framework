...
import com.acooly.openapi.framework.serviceimpl.persistent.dao.ApiServiceTypeDao;
...
public class ApiServiceTypeServiceImpl extends EntityServiceImpl<ApiServiceType, ApiServiceTypeDao>
    implements ApiServiceTypeService {
      ...
      for (Map.Entry<Long, ApiServiceType> entry : dtoMap.entrySet()) {
        ...
        if (node.getParentId() == null || node.getParentId() == 0) {
          ...
          if (dtoMap.get(node.getParentId()) != null) {
            ...
            Collections.sort(
                dtoMap.get(node.getParentId()).getChildren(), new ApiServiceType.NodeComparator());
          }
        }
      }
      types.clear();
