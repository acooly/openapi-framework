 ...
 * <li>明文长度不能超过密钥长度。比如 Java 默认的RSA加密实现不允许明文长度超过密钥长度减去 11(单位是字节，也就是
 * byte)。也就是说，如果我们定义的密钥(我们可以通过 java.security.KeyPairGenerator.initialize(int
 * keysize) 来定义密钥长度)长度为1024(单位是位，也就是 bit)，生成的密钥长度就是 1024位 / 8位/字节 =
 * 128字节，那么我们需要加密的明文长度不能超过 128字节 - 11 字节 = 117字节。也就是说，我们最大能将 117
 * 字节长度的明文进行加密，否则会出问题(抛诸如 javax.crypto.IllegalBlockSizeException: Data must not
 * be longer than 53 bytes 的异常)。
 * <li>BC(Provider --> Security.addProvider(new BouncyCastleProvider());)
 * 提供的加密算法能够支持到的 RSA 明文长度最长为密钥长度。
 * </p>
 *
 * @author zhangpu
 */
public class RSA {
     ...
     * @return 签名的二进制结果
     */
    public static byte[] sign(@NotNull byte[] dataBytes, @NotNull PrivateKey privateKey,
    ...
    public static String signHex(@NotNull String data, @NotNull PrivateKey privateKey, @Null String signatureAlgorithm,
    ...
    public static boolean verifyBase64(String text, String signBase64, PublicKey publicKey, String signatureAlgorithm,
                                       String charset) {
    ...
    public static boolean verifyHex(String text, String signHex, PublicKey publicKey, String signatureAlgorithm,
                                    String charset) {
        byte[] data = getBytes(text, charset);
    ...
    public static String encryptByPublicKeyBase64(String plainText, PublicKey publicKey, String charset) {
    ...
    public static String encryptByPublicKeyHex(String plainText, PublicKey publicKey, String charset) {
    ...
    public static String decryptByPrivateKeyBase64(String encryptedBase64, PrivateKey privateKey, String charset) {
    ...
    public static String decryptByPrivateKeyHex(String plainHex, PrivateKey privateKey, String charset) {
     ...
     * @return
     */
    public static PrivateKey loadPrivateKeyFromKeyStore(String keystoreUri, String keystoreType,
    ...
    public static PrivateKey loadPrivateKey(File privateKeyFile, String keyAlgorithm, String providerName) {
        ...
        try {
            privateKeyBytes = Encodes.decodeBase64(FileUtils.readFileToString(privateKeyFile, Charset.defaultCharset()));
        } catch (Exception e) {
     ...
     * @return
     */
    public static PublicKey loadPublicKey(byte[] publicKeyBytes, String keyAlgorithm, String providerName) {
        String algorithm = Strings.isBlankDefault(keyAlgorithm, DEFAULT_KEY_ALGO);
        try {
            // KEY_ALGORITHM 指定的加密算法
            KeyFactory keyFactory = null;
     ...
     * @return
     */
    public static PublicKey loadPublicKey(File publicKeyFile, String keyAlgorithm, String providerName) {
        byte[] publicKeyBytes = null;
        try {
            publicKeyBytes = Encodes.decodeBase64(FileUtils.readFileToString(publicKeyFile,Charset.defaultCharset()));
        } catch (Exception e) {
     ...
     * @return
     */
    public static PublicKey loadPublicKeyFromKeyStore(String keystoreUri, String keystoreType,
                                                      String keystorePassword) {
        InputStream in = null;
        try {
            KeyStore keyStore = KeyStore.getInstance(keystoreType);
